...
inheritance #in the case where there is an is-a relationship (e.g. panda is a bear) the specific class could inherit from the more 
...
dict_name[new_key] = new_value #adds a dictionary entry  
del dict_name[key_name] #removes a dictionary entry  
dict_name[key] = new_value #assigns a new value to a key
dict_name[key]#you can go to any key by typing
...
```python
my_file.write("Data to be written") #edite a file
write() #takes string argument   
file_name.write(“string” + “*_n”) #in order to write values on separate lines where _ is a backwards slash
...
.append() #adds to a list (takes one argument)  
...
list.pop(index) #removes the item at index from the list and return it to you  
del(list[1]) #like .pop in that it will remove the item at the given index, but it won't return it  
...
else # will execute anytime the loop condition is evaluated to False  
...
```python
#backwards slash fixes a string, placed before an apostrophe to stop a string from ending  
...
```python
, #after our print statement means that our next print statement keeps printing on the same line  
